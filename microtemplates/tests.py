...
class EachTests(unittest.TestCase):
    ...
    def test_each_iterable_in_context(self):
        rendered = Template('{% each items %}<div>{{it}}</div>{% endeach %}').render(items=['alex', 'maria'])
        self.assertEquals(rendered, '<div>alex</div><div>maria</div>')

    def test_each_iterable_as_literal_list(self):
        rendered = Template('{% each [1, 2, 3] %}<div>{{it}}</div>{% endeach %}').render()
        self.assertEquals(rendered, '<div>1</div><div>2</div><div>3</div>')

    def test_each_parent_context(self):
        rendered = Template('{% each [1, 2, 3] %}<div>{{..name}}-{{it}}</div>{% endeach %}').render(name='jon doe')
    ...
    def test_each_no_tags_inside(self):
        rendered = Template('{% each [1,2,3] %}<br>{% endeach %}').render()
        self.assertEquals(rendered, '<br><br><br>')

    def test_nested_objects(self):
        context = {'lines': [{'name': 'l1'}], 'name': 'p1'}
        rendered = Template('<h1>{{name}}</h1>{% each lines %}<span class="{{..name}}-{{it.name}}">{{it.name}}</span>{% endeach %}').render(**context)
        self.assertEquals(rendered, '<h1>p1</h1><span class="p1-l1">l1</span>')

    def test_nested_tag(self):
        rendered = Template('{% each items %}{% if it %}yes{% endif %}{% endeach %}').render(items=['', None, '2'])
...
class IfTests(unittest.TestCase):
    ...
    def test_simple_if_is_true(self):
        rendered = Template('{% if num > 5 %}<div>more than 5</div>{% endif %}').render(num=6)
        self.assertEquals(rendered, '<div>more than 5</div>')

    def test_simple_if_is_false(self):
        rendered = Template('{% if num > 5 %}<div>more than 5</div>{% endif %}').render(num=4)
        self.assertEquals(rendered, '')

    def test_if_else_if_branch(self):
        rendered = Template('{% if num > 5 %}<div>more than 5</div>{% else %}<div>less than 5</div>{% endif %}').render(num=6)
        self.assertEquals(rendered, '<div>more than 5</div>')

    def test_if_else_else_branch(self):
        rendered = Template('{% if num > 5 %}<div>more than 5</div>{% else %}<div>less or equal to 5</div>{% endif %}').render(num=4)
        self.assertEquals(rendered, '<div>less or equal to 5</div>')

    def test_nested_if(self):
        tmpl = '{% if num > 5 %}{% each [1, 2] %}{{it}}{% endeach %}{% else %}{% each [3, 4] %}{{it}}{% endeach %}{% endif %}'
    ...
    def test_truthy_thingy(self):
        rendered = Template('{% if items %}we have items{% endif %}').render(items=[])
        self.assertEquals(rendered, '')
        rendered = Template('{% if items %}we have items{% endif %}').render(items=None)
        self.assertEquals(rendered, '')
        rendered = Template('{% if items %}we have items{% endif %}').render(items='')
        self.assertEquals(rendered, '')
        rendered = Template('{% if items %}we have items{% endif %}').render(items=[1])
